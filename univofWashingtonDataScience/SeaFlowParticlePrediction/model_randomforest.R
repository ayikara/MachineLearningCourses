...
fol <- formula(pop~ fsc_small + fsc_perp + fsc_big + pe + chl_big + chl_small)
model <- randomForest(fol, data=train.testrb)

# Make predictions:
predictRF = predict(model, newdata=test.testrb)

table(test.testrb$pop, predictRF)

accRF =( 49+ 5578+10084 +9069 +8551)/nrow(test.testrb)
